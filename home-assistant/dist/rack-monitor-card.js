...
`,lt=t=>t&&"unavailable"!==t.state&&"unknown"!==t.state;function ht(t,e){e&&t.dispatchEvent(new CustomEvent("hass-more-info",{detail:{entityId:e},bubbles:!0,composed:!0}))}function dt(t,e){t.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0}))}function pt(t){const e={...t};for(const t of Object.keys(e))""!==e[t]&&void 0!==e[t]||delete e[t];return e}const mt=["zone1_entity","zone2_entity","intake_entity","fan1_entity","fan1_rpm_entity","fan2_entity","fan2_rpm_entity","mode_entity"],_t={temp_min:20,temp_max:50,temp_warn:38,temp_crit:45};class ut extends ot{static properties={hass:{attribute:!1},_config:{state:!0}};setConfig(t){if(!t.zone1_entity||!t.zone2_entity||!t.intake_entity)throw new Error("rack-monitor-card: zone1_entity, zone2_entity and intake_entity are required");this._config={..._t,...t}}static getConfigElement(){return document.createElement("rack-monitor-card-editor")}static getStubConfig(t){const e=t?Object.keys(t.states):[],s=t=>e.find(e=>t.test(e))||"";return{title:"Rack Monitor",zone1_entity:s(/^sensor\..*zone_?1.*temp|^sensor\..*temp.*zone_?1/),zone2_entity:s(/^sensor\..*zone_?2.*temp|^sensor\..*temp.*zone_?2/),intake_entity:s(/^sensor\..*intake/),fan1_entity:s(/^fan\..*fan_?1/),fan1_rpm_entity:s(/^sensor\..*fan_?1.*rpm/),fan2_entity:s(/^fan\..*fan_?2/),fan2_rpm_entity:s(/^sensor\..*fan_?2.*rpm/),mode_entity:s(/^select\..*(mode|modus)/)}}getCardSize(){return 4}_stateObj(t){const e=this._config?.[t];return e?this.hass?.states[e]:void 0}_temp(t){const e=parseFloat(t?.state);return Number.isFinite(e)?e.toFixed(1):"–"}_unit(t){return t?.attributes?.unit_of_measurement??"°C"}_rpm(t){const e=parseFloat(t?.state);return Number.isFinite(e)?Math.round(e).toString():"–"}_pwm(t){if(!lt(t))return"–";if("off"===t.state)return"0";const e=t.attributes?.percentage;return Number.isFinite(e)?Math.round(e).toString():"–"}_online(){const t=this._stateObj("status_entity");return t?"on"===t.state:["zone1_entity","zone2_entity","intake_entity"].every(t=>lt(this._stateObj(t)))}_lastUpdated(){let t;for(const e of mt){const s=this._stateObj(e);if(s?.last_updated){const e=new Date(s.last_updated);(!t||e>t)&&(t=e)}}return t}_setMode(t){const e=this._stateObj("mode_entity");e&&e.state!==t&&this.hass.callService("select","select_option",{entity_id:this._config.mode_entity,option:t})}_label(t,e){return this._config?.[`${t.replace("_entity","")}_name`]??e}render(){if(!this.hass||!this._config)return B;const t=this._online(),e=this._lastUpdated(),s=this._stateObj("mode_entity");return I`
    ...
    `}_hasFans(){return this._config.fan1_entity||this._config.fan2_entity}_tempSeverity(t){return Number.isFinite(t)?t>=this._config.temp_crit?"crit":t>=this._config.temp_warn?"warn":"ok":"ok"}_renderTemp(t,e){const s=this._stateObj(t),i=parseFloat(s?.state),{temp_min:n,temp_max:r}=this._config,o=Number.isFinite(i)?(i-n)/(r-n):0;return I`
      <div
      ...
      >
        ...
        <div class="bar">
          <div
            class="bar-fill ${this._tempSeverity(i)}"
            style="width: ${Math.max(0,Math.min(100,100*o))}%"
          ></div>
        </div>
      </div>
    ...
    `]}customElements.define("rack-monitor-card",ut);const ft=[{name:"title",selector:{text:{}}},{name:"temperatures",type:"expandable",flatten:!0,expanded:!0,schema:[{name:"zone1_entity",required:!0,selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"zone2_entity",required:!0,selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"intake_entity",required:!0,selector:{entity:{domain:"sensor",device_class:"temperature"}}}]},{name:"fans",type:"expandable",flatten:!0,expanded:!0,schema:[{name:"",type:"grid",schema:[{name:"fan1_entity",selector:{entity:{domain:"fan"}}},{name:"fan1_rpm_entity",selector:{entity:{domain:"sensor"}}},{name:"fan2_entity",selector:{entity:{domain:"fan"}}},{name:"fan2_rpm_entity",selector:{entity:{domain:"sensor"}}}]}]},{name:"control",type:"expandable",flatten:!0,schema:[{name:"mode_entity",selector:{entity:{domain:"select"}}},{name:"status_entity",selector:{entity:{domain:"binary_sensor",device_class:"connectivity"}}}]},{name:"thresholds",type:"expandable",flatten:!0,schema:[{name:"",type:"grid",schema:[{name:"temp_warn",selector:{number:{min:0,max:120,mode:"box",unit_of_measurement:"°C"}}},{name:"temp_crit",selector:{number:{min:0,max:120,mode:"box",unit_of_measurement:"°C"}}},{name:"temp_min",selector:{number:{min:0,max:120,mode:"box",unit_of_measurement:"°C"}}},{name:"temp_max",selector:{number:{min:0,max:120,mode:"box",unit_of_measurement:"°C"}}}]}]}],yt={title:"Title",temperatures:"Temperatures",fans:"Fans",control:"Control & status",zone1_entity:"Zone 1 temperature",zone2_entity:"Zone 2 temperature",intake_entity:"Intake temperature",fan1_entity:"Fan 1",fan1_rpm_entity:"Fan 1 RPM sensor",fan2_entity:"Fan 2",fan2_rpm_entity:"Fan 2 RPM sensor",mode_entity:"Mode select",status_entity:"Status (connectivity)",thresholds:"Thresholds",temp_warn:"Temp warn",temp_crit:"Temp critical",temp_min:"Temp bar scale min",temp_max:"Temp bar scale max"},$t={fan1_entity:"Fan entity, used for the PWM duty (%)",fan2_entity:"Fan entity, used for the PWM duty (%)",mode_entity:"Optional. Shows the mode bar (Off / On / Auto / …)",status_entity:"Optional. Falls back to sensor availability if empty"};class gt extends ot{static properties={hass:{attribute:!1},_config:{state:!0}};setConfig(t){this._config=t}_computeLabel=t=>yt[t.name]??t.name;_computeHelper=t=>$t[t.name]??"";_valueChanged(t){t.stopPropagation(),dt(this,pt(t.detail.value))}render(){return this.hass&&this._config?I`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${ft}
